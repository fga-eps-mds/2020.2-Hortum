...
class ComplaintSerializer(serializers.ModelSerializer):
    ...
    def validate_emailProductor(self, emailProductor):
        if Complaint.objects.filter(emailCustomer=self.context['request'].user, idProductor__user__email=emailProductor).exists():
...
class ComplaintDeleteSerializer(serializers.ModelSerializer):
    ...
    def validate_emailProductor(self, emailProductor):
        if not self.context['view'].get_queryset().filter(idProductor__user__email=emailProductor).exists():
